...
export const EmptyState = ({ icon: Icon, title, message, actionText, onAction, actionColor = "bg-blue-600 hover:bg-blue-500" }) => (
  <div className="flex flex-col items-center justify-center h-full p-8 bg-surface-200 rounded-xl border border-surface-400 text-center w-full shadow-sm">
    ...
    {actionText && onAction && (
      <button
        onClick={onAction}
        className={`px-6 py-2.5 ${actionColor} text-white font-bold rounded-xl transition-colors shadow-sm`}
      >
        {actionText}
      </button>
    )}
